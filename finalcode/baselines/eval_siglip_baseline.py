...
print(f"\n配置:")
print(f"  模型: SigLIP-base-patch16-224")
...
processor = AutoProcessor.from_pretrained(
    ...
    cache_dir="/Users/jiazhu/.cache/huggingface"
)
...
def process_dataset(data_dir, dataset_json, dataset_name, is_rsitmd=False):
    ...
    # 使用完整的test split（COCO: 5000张，RSITMD: 全部）
    ...
    for img_data in tqdm(test_images, desc="多视角"):
        try:
            ...
            # 用augmentor生成augmented视角
            augmented_imgs_tensor = augmentor.augment(image, IMAGE_AUGS)  # [n_augs, 3, 224, 224]
            
            # 将tensor转回PIL用processor处理
            augmented_imgs_pil = []
            for aug_tensor in augmented_imgs_tensor:
                # 反归一化
                aug_tensor = aug_tensor * 0.5 + 0.5  # 从[-1,1]转到[0,1]
            ...
            with torch.no_grad():
                # 使用get_image_features获取投影后的特征
                outputs = model.get_image_features(**inputs)
                multiview_features = outputs.pooler_output if hasattr(outputs, 'pooler_output') else outputs
    ...
    print(f"  成功: {len(multiview_image_features_list)}, 失败: {error_count}")
    
    if len(multiview_image_features_list) == 0:
        raise RuntimeError(f"没有成功处理任何图像！请检查数据路径和图像文件。")
    ...
    for template in tqdm(PROMPT_TEMPLATES, desc="文本"):
        ...
        inputs = processor(text=wrapped_texts, return_tensors="pt", padding=True, truncation=True).to(device)
        with torch.no_grad():
            # 使用get_text_features获取投影后的特征
    ...
    for img_data in tqdm(test_images, desc="单视角", leave=False):
        try:
            if is_rsitmd:
                img_path = data_dir / "images" / img_data['filename']
            else:
            ...
            with torch.no_grad():
                # 使用get_image_features获取投影后的特征
                outputs = model.get_image_features(**inputs)
                image_feature = outputs.pooler_output if hasattr(outputs, 'pooler_output') else outputs
                image_feature = F.normalize(image_feature, dim=-1).cpu()
            single_view_features_list.append(image_feature.squeeze(0))
            
            # 为当前图像添加文本
            captions = [sent['raw'] for sent in img_data['sentences']]
            baseline_text_list.extend(captions)
            baseline_image_ids.extend([len(single_view_features_list)-1] * len(captions))
    ...
    print(f"  Baseline成功: {len(single_view_features_list)}, 失败: {baseline_error_count}")
    
    if len(single_view_features_list) == 0:
        raise RuntimeError("Baseline没有成功处理任何图像！")
    
    single_view_features = torch.stack(single_view_features_list)
    baseline_image_ids_tensor = torch.tensor(baseline_image_ids)
    
    print(f"  DEBUG: single_view_features shape = {single_view_features.shape}")
    print(f"  DEBUG: baseline_text_list length = {len(baseline_text_list)}")
    print(f"  DEBUG: baseline_image_ids_tensor shape = {baseline_image_ids_tensor.shape}")
    print(f"  DEBUG: baseline_image_ids_tensor unique values = {baseline_image_ids_tensor.unique().shape[0]}")
    
    inputs = processor(text=baseline_text_list, return_tensors="pt", padding=True, truncation=True).to(device)
    with torch.no_grad():
        # 使用get_text_features获取投影后的特征
        outputs = model.get_text_features(**inputs)
        single_text_features = outputs.pooler_output if hasattr(outputs, 'pooler_output') else outputs
        single_text_features = F.normalize(single_text_features, dim=-1).cpu()
    
    print(f"  DEBUG: single_text_features shape = {single_text_features.shape}")
    print(f"  DEBUG: single_text_features mean = {single_text_features.mean().item():.6f}")
    print(f"  DEBUG: single_text_features std = {single_text_features.std().item():.6f}")
    print(f"  DEBUG: First similarity = {(single_view_features[0] @ single_text_features.T).max().item():.4f}")
    
    results_baseline = evaluate_full(single_view_features, single_text_features, baseline_image_ids_tensor, "Baseline")
    ...
    # 一致性正则化
    print(f"\n阶段3: + 一致性正则化")
    img_projector = ConsistencyProjector(input_dim=FEATURE_DIM, hidden_dim=PROJ_HIDDEN_DIM).to(device)
    txt_projector = ConsistencyProjector(input_dim=FEATURE_DIM, hidden_dim=PROJ_HIDDEN_DIM).to(device)
    ...
    for epoch in range(PROJ_EPOCHS):
        ...
        for batch_start in range(0, n_images, BATCH_SIZE):
            ...
            proj_txt_multi = txt_projector(batch_txt_multi.reshape(-1, FEATURE_DIM)).reshape(len(batch_txt_indices), len(PROMPT_TEMPLATES), FEATURE_DIM)
    ...
    # 伪标签自训练
    print(f"\n阶段4: + 伪标签自训练")
    img_adapter = SelfTrainingAdapter(input_dim=FEATURE_DIM, hidden_dim=ADAPTER_HIDDEN_DIM).to(device)
    txt_adapter = SelfTrainingAdapter(input_dim=FEATURE_DIM, hidden_dim=ADAPTER_HIDDEN_DIM).to(device)
    ...
    # 最终对比
    print(f"\n{'='*80}")
    print(f"{dataset_name}最终结果 - SigLIP")
    print(f"{'='*80}")
